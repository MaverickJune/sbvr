...
def all_gather_dict(dict: dict) -> list:
    """
    Gather a dictionary from all processes in the distributed group.
    Args:
        dict (dict): The dictionary to gather.
    Returns:
        list: A list of dictionaries, one for each process in the group.
    """
    world_size = get_world_size()
    gathered_dicts = [None] * world_size
    torch.distributed.all_gather_object(gathered_dicts, dict)
...
def get_world_size() -> int:
    """
    Get world size using torch.distributed if available. Otherwise, the WORLD_SIZE env var instead if initialized.
    Returns 1 if neither condition is met.
    """
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        return torch.distributed.get_world_size()

    environ_world_size = os.environ.get("WORLD_SIZE", "")
    if environ_world_size.isdecimal():
