...
class LlamaSbvrFlashAttention2(LlamaSbvrAttention):
    """
...
class LlamaSbvrSdpaAttention(LlamaSbvrAttention):
    """
...
class LlamaSbvrDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.mlp = LlamaSbvrMLP(config=config, layer_idx=layer_idx)
...
class LlamaSbvrModel(LlamaPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [
                LlamaSbvrDecoderLayer(config, layer_idx)
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                raise NotImplementedError("Sbvrized model does not support training")
            else:
...
class LlamaForSbvrLM(LlamaPreTrainedModel):
