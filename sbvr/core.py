...
def input_sbvr_mm_T(input_bvr, input_coeff_idx, input_coeff_set,
                    w_sbvr,
                    bias=None):
    if bias is None:
        bias = w_sbvr._get_dummy_bias()
...
class sbvrizer(torch.nn.Module):
    def __init__(self, bvr_len, num_sums, set_size=4):
        super(sbvrizer, self).__init__()
        self.bvr_len = bvr_len
        self.bvr_dtype = torch.uint32
        self.num_sums = num_sums
        self.set_size = set_size
        
        self.coeff_set = torch.nn.Parameter(torch.zeros((set_size, num_sums), dtype=torch.bfloat16), requires_grad=False)
        
    def load_coeff_set(self, coeff_set):
        coeff_set = coeff_set.to(self.coeff_set.device).to(self.coeff_set.dtype)
        with torch.no_grad():
            self.coeff_set.copy_(coeff_set)
    ...
    @torch.inference_mode()
    def _get_candidate_mm_ext(self):
        device = self.coeff_set.device
        dtype = self.coeff_set.dtype
        if not hasattr(self, 'bin_combs'):
            self.bin_combs = torch.tensor(
                list(itertools.product([0, 1], repeat=self.num_sums)),
                dtype=dtype, device=device
    ...
    def forward(self, x, mode=0):
        """
        mode 0: return x
        mode 1: perform sbvr encoding with naive pytorch, return x
        mode 2: perform sbvr encoding with customized cuda kernel, return x
        """
        if mode == 0:
            return x
        
        elif mode == 1:
            # cleanup the previous encoding info
            self.clean_encode_info(release_memory=False)
            
            # assign x to data, just to reuse the code
            data = x
            ...
            # perform coeff searching
            candidate_matrix = self._get_candidate_mm_ext()
            n_ss_row = candidate_matrix.shape[0]
            n_ss_col = candidate_matrix.shape[1]
            candidate_matrix = candidate_matrix.view(1, n_ss_row, 1, n_ss_col)
            ...
            raise NotImplementedError(r_str("Customized cuda kernel is not implemented yet"))
        
        else:
            raise ValueError(f"Invalid mode: {mode}, mode should be 0, 1 or 2")
