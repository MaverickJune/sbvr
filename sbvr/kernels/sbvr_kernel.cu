    ...
    int cgroup_per_inner_vec)
{
    // Tensor shapes:
    // l_bvr: 
    //  [out_rows / 4, self.num_sums, cgroup_per_inner_vec, 4, bvr_per_cgroup]
    // l_coeff_idx: [num_cgroups]
    // l_coeff_cache: [cache_size, num_sums]
    // r_bvr: 
    //  [out_cols / 4, self.num_sums, cgroup_per_inner_vec, 4, bvr_per_cgroup]
            ...
            for (int l_idx = 0; l_idx < l_num_sums; l_idx++)
            {
                for (int r_idx = 0; r_idx < r_num_sums; r_idx++)
                {
                    for (int bvr_idx = 0; bvr_idx < BVR_PER_CGROUP; bvr_idx++)
                    {
                        // [out_rows / 4, self.num_sums, cgroup_per_inner_vec, 4, bvr_per_cgroup]
                        uint32_t l = l_bvr[
                            (row / 4) * l_num_sums * cgroup_per_inner_vec * 4 *
                            BVR_PER_CGROUP  +
                            l_idx * cgroup_per_inner_vec * 4 * BVR_PER_CGROUP + 
                            cg_idx * 4 * BVR_PER_CGROUP + 
                            (row % 4) * BVR_PER_CGROUP +
                            bvr_idx];
                        uint32_t r = r_bvr[
                            (col / 4) * r_num_sums * cgroup_per_inner_vec * 4 *
                            BVR_PER_CGROUP +
                            r_idx * cgroup_per_inner_vec * 4 * BVR_PER_CGROUP +
                            cg_idx * 4 * BVR_PER_CGROUP + 
                            (col % 4) * BVR_PER_CGROUP + 
                            bvr_idx];
                        uint32_t lr = l & r;
...
template <typename LIndexT, typename RIndexT>
__global__ void cuda_4x1_sbvr_mm_T(
    uint32_t* l_bvr,
    LIndexT* l_coeff_idx,
    __half* l_coeff_cache,
    uint32_t* r_bvr,
