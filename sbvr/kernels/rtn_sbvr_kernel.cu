...
constexpr int MAX_NUM_GROUPS   = 128; // 128 * 128 = 16384 > K
    ...
    int device_id = 0)
{
    ...
    int num_groups = K / GROUP_SIZE; // K/128
    int num_blocks = (num_groups + GROUPS_PER_BLOCK - 1) / GROUPS_PER_BLOCK;
    dim3 threads = {_1xtN_tN, THREAD_PER_WARP / _1xtN_tN, WARP_PER_BLOCK};
    dim3 blocks_input(num_blocks);

    int blocks_mm = cuda_prop_list[device_id].multiProcessorCount * BLOCK_PER_SM;

    fused_rtn7_lut_bvr_warp<<<blocks_input, threads>>>(x, d_bvr_ptr, d_scales_ptr, num_groups);

    rtn_7_sbvr_1xtN_mm_T<RIndexT, RNumSums, _1xtN_tN><<<blocks_mm, threads>>>(
