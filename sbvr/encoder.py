...
class sbvr_encoder():
    def __init__(self, **kwargs):
        ...
        self.bvr_len = kwargs.get("bvr_len", 128)
        ...
        # (wjbang, 2026.03.06), added config for bias setting
        self.has_bias = kwargs.get("has_bias", False)
        self.bias_shape = kwargs.get("bias_shape", None)
    ...
    def _get_result_str(self):
        avg_mse = sum(self.mse_history) / len(self.mse_history) * 0.99 \
            if self.mse_history else 0
        result_str = y_str("\tAverage MSE: ") + str(avg_mse) + \
